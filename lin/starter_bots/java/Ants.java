...
public class Ants {
	...
	private Set<Tile> myHillList = new HashSet<Tile>();
  private Set<Tile> opHill = new HashSet<Tile>();
  public  List<Tile> opHillList = new ArrayList();
  public Tile deadHill = new Tile(0, 0);
  
  public int mapVisit[][] = new int[200][200];

  public void initMap(int n, int m) {
    mapVisit = new int[n][m];
    for(int i = 0 ; i < n ; i ++)
      for(int j = 0 ; j < m ; j++)
        mapVisit[i][j] = 0;//nevizitate;
  }
  ...
  public int turn() {
	...
	public long player_seed() {
		return this.player_seed;
	}

	public Set<Tile> myHillList(){
		return this.myHillList;
	}

  public Set<Tile> opHill(){
    return this.opHill;
  }

  public boolean atention(Tile dead){
    this.deadHill = new Tile(dead.row(), dead.col());
    return true;
  }

	public List<Tile> opHillList(){
    this.opHillList.clear();
    Iterator <Tile>itHill = opHill.iterator();
    while(itHill.hasNext()){
      Tile t = itHill.next();
      this.opHillList.add(t);
    }
    if(deadHill.row() == 0 && deadHill.col() == 0)
      return this.opHillList;
    else{
      if(this.opHillList.get(0).row() == this.deadHill.row() && this.opHillList.get(0).col() == this.deadHill.col())
        this.opHillList.remove(0);
      else
        this.opHillList.remove(1);
      return this.opHillList;
    }
  }
				...
				if (tokens[0].equals("w")) {
			    	...
			    	this.deadList.add(new Tile(row, col));
						...
						if(tokens[3].equals("0"))
							this.myHillList.add(new Tile(row, col));
						else
							this.opHill.add(new Tile(row, col));
					}
			}
		}
		return true;
  ...
  public int distRow(Tile t1, Tile t2) {
    int dRow = Math.abs(t1.row() - t2.row());
    return Math.min(dRow, this.rows - dRow);
  }

  public int distCol(Tile t1, Tile t2) {
    int dCol = Math.abs(t1.col() - t2.col());
    return Math.min(dCol, cols - dCol);
  }

  public boolean isAlphaDist(Tile tile1, Tile tile2) {
    int dx = distRow(tile1, tile2);
    int dy = distCol(tile1, tile2);
    return (dy <= 1 && dx == 2) || (dy == 2 && dx <= 1);
  }
	
	public int my_distance (Tile t1, Tile t2){
		int dRow = Math.abs(t1.row() - t2.row());
		int dCol = Math.abs(t1.col() - t2.col());
